...
# Faire une fonction qui tweet un message à 5 utilisatuer choisi au hasard dans un tableau
...
# Liste de noms d'utilisateurs Twitter
...
# Prend au hasard 5 utilisateurs 
    journalist_handles_random = journalist_handles.sample(5)

# Tweet leur une phrase
    journalist_handles_random.each do |journalist_handle|
        p journalist_handle
    # client.update("#{journalist_handle} Bonjour, je trouve ton travail génial! #bonjour_monde => @the_hacking_pro")
    end
end

# Maintenant que tu as dit bonjour à Internet, nous allons te demander de follow les derniers tweets en fonction d'un hashtag 
...
def search_twitter
    client = Twitter::REST::Client.new do |config|
        ...
        client.search("to:justinbieber marry me", result_type: "recent").take(3).each do |tweet|
            ...
            to_follow.each do |to_following|
                client.follow("#{to_following}")
            end    
        end 
